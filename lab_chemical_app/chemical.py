...
@requires_permission('chemical', 'list')
def export_excel():
    """Export filtered chemical analyses to xlsx, respecting current filters."""
    import xlsxwriter

    query, _ = _filtered_query(request.args)
    rows = query.order_by(ChemicalAnalysis.test_date.desc(),
                          ChemicalAnalysis.ladle_no.desc()).all()

    buf = BytesIO()
    wb = xlsxwriter.Workbook(buf, {"in_memory": True})
    ws = wb.add_worksheet("Chemical")
    bold = wb.add_format({"bold": True, "bg_color": "#D3D3D3"})

    headers = ["Date", "Furnace", "Ladle#", "Ladle ID", "C", "Si", "Mg", "S", "CE", "Decision"]
    for c, h in enumerate(headers):
        ws.write(0, c, h, bold)

    for r, a in enumerate(rows, start=1):
        ws.write(r, 0, str(a.test_date) if a.test_date else "")
        ws.write(r, 1, a.furnace.furnace_code if a.furnace else "")
        ws.write(r, 2, a.ladle_no)
        ws.write(r, 3, a.ladle_id or "")
        ws.write(r, 4, a.carbon)
        ws.write(r, 5, a.silicon)
        ws.write(r, 6, a.magnesium)
        ws.write(r, 7, a.sulfur)
        ws.write(r, 8, a.carbon_equivalent)
        ws.write(r, 9, a.decision or "")
    ...
    return send_file(
        ...
        as_attachment=True,
        download_name="chemical_analyses.xlsx",
    )
