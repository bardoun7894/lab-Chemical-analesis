...
MODULES = {
    ...
    # The storekeeper's receiving screen. Its own module rather than another
    # `stages` screen, because `stages.edit` is all-or-nothing across the
    # console, the edit form and the per-stage AJAX save — granting any of it
    # would reach far past the four delivery fields.
    'warehouse': {
        'scan': ACTION_READ, 'receive': ACTION_WRITE,
    },
    # One key per report screen. The blanket 'view' key is kept only for the
    # JSON/AI endpoints that back a page rather than being one, so revoking a
    # single report never silently disables another. Adding a report here is
    # enough: seed_default_permissions() creates the row on the next boot and
    # _DEFAULT_REPORT_SCREENS grants it to the non-admin roles.
    'reports': {
        'list': ACTION_READ, 'view': ACTION_READ, 'export': ACTION_EXPORT,
        # Hubs
        'bi_dashboard': ACTION_READ, 'management_dashboard': ACTION_READ,
        # Production
        'production_summary': ACTION_READ, 'order_performance': ACTION_READ,
        'customer_production': ACTION_READ, 'delivery_report': ACTION_READ,
        'daily_production': ACTION_READ, 'annealing_hourly': ACTION_READ,
        # Quality
        'chemical_analysis': ACTION_READ, 'defect_summary': ACTION_READ,
        'non_conformance': ACTION_READ, 'rft_report': ACTION_READ,
        'rework_report': ACTION_READ, 'defect_analysis': ACTION_READ,
        'defect_heatmap': ACTION_READ, 'heat_traceability': ACTION_READ,
        'traceability_tree': ACTION_READ, 'andon': ACTION_READ,
        'mechanical_statistical': ACTION_READ, 'mechanical_properties': ACTION_READ,
        'lab_trends': ACTION_READ, 'data_quality': ACTION_READ,
        'diagnosis': ACTION_READ, 'approvals': ACTION_READ,
        'approver_comparison': ACTION_READ,
        # Performance
        'stage_performance': ACTION_READ, 'machine_performance': ACTION_READ,
        'mold_performance': ACTION_READ, 'weight_saving': ACTION_READ,
        'saving_matrix': ACTION_READ, 'stage_cycle_time': ACTION_READ,
        'stage_funnel': ACTION_READ, 'stage_audit': ACTION_READ,
        'stage_measurements': ACTION_READ,
        # Shift & delivery
        'shift_engineer': ACTION_READ, 'shift_engineer_comparison': ACTION_READ,
        'delivery_overview': ACTION_READ, 'delivery_comparison': ACTION_READ,
        # Comparisons
        'ytd_comparison': ACTION_READ, 'period_compare': ACTION_READ,
        # SPC / capability are quality-engineering screens: admin + super_admin
        # only by default; other roles must be granted explicitly in the matrix.
        'spc': ACTION_READ, 'capability': ACTION_READ,
    },
    'stickers': {
    ...
    # Certificates go out to the customer under the company's name, so issuing
    # one is deliberately a separate module from the order screens it draws its
    # data from.
    'certificates': {
        'list': ACTION_READ, 'add': ACTION_CREATE, 'edit': ACTION_WRITE,
        'delete': ACTION_DELETE, 'print': ACTION_EXECUTE,
        'reissue': ACTION_EXECUTE,
    },
    'analytics': {
    ...
    'chatbot': {
        ...
        'agent': ACTION_EXECUTE, 'sql': ACTION_EXECUTE,
    },
    ...
    'admin': {
        ...
        'dimension_standards_list': ACTION_READ,
        ...
        'reason_types_list': ACTION_READ, 'reason_types_add': ACTION_CREATE,
        'reason_types_edit': ACTION_WRITE, 'reason_types_delete': ACTION_DELETE,
        'defect_reasons_list': ACTION_READ, 'defect_reasons_add': ACTION_CREATE,
        'defect_reasons_edit': ACTION_WRITE, 'defect_reasons_delete': ACTION_DELETE,
        ...
        # Backups: taking one is safe (execute); deleting one destroys a
        # recovery point, so it is a separate delete verb.
        'backups_create': ACTION_EXECUTE,
        'backups_delete': ACTION_DELETE,
    },
}

# Report screens auto-granted to the non-admin roles. Derived from MODULES so a
# newly added report behaves like it did under the old blanket 'view' key;
# revoking access is then a matrix decision, not a code change. SPC and
# capability are excluded on purpose — they stay admin-only until granted.
_DEFAULT_REPORT_SCREENS = [
    screen for screen in MODULES['reports']
    if screen not in ('spc', 'capability', 'export')
]
...
ROLE_DEFAULTS = {
    ...
    'operator': {
        ...
        'stages': ['list', 'add', 'edit', 'tracking', 'delivery', 'shift_dashboard'],
        'reports': _DEFAULT_REPORT_SCREENS,
    ...
    'warehouse': {
        ...
        'main': ['dashboard'],
        # Nothing else on purpose: no stages, no reports, no stickers.
    },
    'viewer': {
        'chemical': ['list'],
        'mechanical': ['list'],
...
def restore_default_grants():
    """Reset every role's grants to ROLE_DEFAULTS — the "undo tampering" path.

    Unlike the seed, which grants defaults only on newly-created rows so a
    deliberate revoke is never silently re-granted, this wipes ALL
    RolePermission rows and re-grants per ROLE_DEFAULTS. After it runs the
    matrix is exactly what a fresh install shows.

    Reconciles the taxonomy first (same as boot) so "default" also covers
    screens added since the DB was created.
