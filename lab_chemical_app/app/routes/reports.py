...
from app.services import pipe_measurement_service
from app.models.production_order import ProductionOrder


def _crosstab_export(filters):
    """Flatten the current crosstab into export rows.

    One row per crosstab row per pivot leaf, carrying every selected measure,
    so the spreadsheet holds the same numbers the screen shows rather than a
    differently-shaped re-query. Weight is already de-duplicated per pipe by
    the service.
    """
    submitted = request.args.get("xt_set") == "1"
    ct = crosstab_service.crosstab(
        filters,
        pivot_dims=request.args.getlist("xt_pivot") if submitted
        else (request.args.getlist("xt_pivot") or ["stage"]),
        row_dims=request.args.getlist("xt_rows") if submitted
        else (request.args.getlist("xt_rows") or ["dn", "pipe_class"]),
        sort=request.args.get("xt_sort", "total_desc"),
        # 0 = no limit: an export that silently dropped rows the screen warned
        # about would be worse than the truncation it came from.
        row_limit=0,
        measures=(request.args.getlist("xt_m")
                  if request.args.get("xt_mset") == "1" else None),
    )

    labels = {k: v[0] for k, v in crosstab_service.DIMENSIONS.items()}
    rows = []
    for row in ct["rows"]:
        base = {labels.get(d, d): lbl
                for d, lbl in zip(ct["row_dims"], row["labels"])}
        pairs = list(zip(ct["leaf_keys"], row["cells"])) or [([], row["total"])]
        for leaf, cell in pairs:
            entry = dict(base)
            for dim, value in zip(ct["pivot_dims"], leaf):
                entry[labels.get(dim, dim)] = value
            for m in ct["measures"]:
                entry[crosstab_service.MEASURES[m]["label"][0]] = cell.get(m)
            rows.append(entry)
    ...
    return {
        ...
        "summary": {
            "rows": ct["total_rows"],
            "facts": ct["fact_count"],
            "pipes_with_both_weights": ct["grand_total"].get("weighed", 0),
            "pipes": ct["grand_total"].get("pipes", 0),
        },
        "filters": filters,
    }
...
def _bi_filter_options():
    """Dropdown options for the BI filter bar, read from the data.

    Learned rather than hard-coded: a DN list in the template rots the moment
    a new diameter is produced, and an approver list has to be whoever actually
    approved something.
    """
    ...
    def _col(column):
        return sorted(
            {v for (v,) in db.session.query(column).distinct().all()
             if v not in (None, "")},
            key=lambda v: (str(type(v)), v),
        )

    approver_ids = [
        i for (i,) in db.session.query(PipeStage.approved_by_id).distinct().all()
        if i
    ]
    approvers = []
    if approver_ids:
        for u in User.query.filter(User.id.in_(approver_ids)).all():
            approvers.append({
                "id": u.id,
                "name": (u.full_name or u.full_name_ar or u.username or
                         str(u.id)),
            })
        approvers.sort(key=lambda a: a["name"])

    return {
        "diameters": _col(Pipe.diameter),
        "classes": _col(Pipe.pipe_class),
        "molds": _col(Pipe.mold_number),
        "engineers": _col(Pipe.shift_engineer),
        "customers": _col(ProductionOrder.customer_name),
        "stages": ProductionStage.active_names(),
        "machines": [
            {"id": m.id, "code": m.machine_code or str(m.id)}
...
def _iso_date_arg(name):
    """One ISO date from the query string, or None for missing/malformed.

    A bad date must degrade to "no date given", never 500 a whole dashboard.
    """
...
@login_required
@requires_permission("reports", "production_summary")
...
@login_required
@requires_permission("reports", "order_performance")
...
@login_required
@requires_permission("reports", "customer_production")
...
@login_required
@requires_permission("reports", "delivery_report")
def delivery_report():
...
@login_required
@requires_permission("reports", "rft_report")
def rft_report():
...
@reports_bp.route("/rework-report")
@login_required
@requires_permission("reports", "rework_report")
def rework_report():
    data = analytics_service.rework_report(
        analytics_service.parse_filters(request.args)
    )
    return _render_analytics_report("rework_report.html", "Rework by Stage", data)


@reports_bp.route("/defect-analysis")
@login_required
@requires_permission("reports", "defect_analysis")
def defect_analysis():
...
@login_required
@requires_permission("reports", "heat_traceability")
...
# ---------------------------------------------------------------------------
# Andon board — the wall screen
# ---------------------------------------------------------------------------

from app.services import andon_service


@reports_bp.route("/andon")
@login_required
@requires_permission("reports", "andon")
def andon():
    """Full-screen shop status for a wall monitor. Refreshes itself."""
    try:
        alert_hours = int(request.args.get("hours", andon_service.DEFAULT_ALERT_HOURS))
    except (TypeError, ValueError):
        alert_hours = andon_service.DEFAULT_ALERT_HOURS
    data = andon_service.board(alert_hours=alert_hours)
    return render_template("reports/andon.html", d=data, alert_hours=alert_hours)


@reports_bp.route("/andon/panel")
@login_required
@requires_permission("reports", "andon")
def andon_panel():
    """The board's body alone — polled so the wall screen never reloads."""
    try:
        alert_hours = int(request.args.get("hours", andon_service.DEFAULT_ALERT_HOURS))
    except (TypeError, ValueError):
        alert_hours = andon_service.DEFAULT_ALERT_HOURS
    data = andon_service.board(alert_hours=alert_hours)
    return render_template("reports/_andon_panel.html", d=data,
                           alert_hours=alert_hours)


# ---------------------------------------------------------------------------
# Traceability canvas — the order's workflow as a wired node graph
# ---------------------------------------------------------------------------
...
def _traceability_context():
    """Resolve what to draw. Three modes share one canvas:

    live  — the production line with what is stuck at each stage (default)
    order — one production order, a band per heat
    pipe  — one pipe cradle to grave, the certificate view
    """
    q = request.args.get("root", "").strip()
    mode = request.args.get("mode", "").strip()
    expand = [t for t in request.args.get("expand", "").split(",") if t]
    problems_only = request.args.get("problems") == "1"

    subject = order = pipe = None
    focus = None
    not_found = False

    if q:
        found, obj, focus = traceability_service.resolve_root(q)
        if found is None:
            not_found = True
            mode = mode or "live"
        elif found == "pipe":
            pipe = obj
            mode = mode or "pipe"
            subject = obj.pipe_code or obj.no_code
        elif found == "ladle":
            order = obj.production_order
            mode = mode or "order"
            subject = f"Ladle {obj.ladle_id}"
            if order is None:
                # a heat with no order still draws, rooted at itself
                graph = traceability_service.build_graph(
                    order=None, ladles=[obj], expand=expand,
                    problems_only=problems_only)
                return {"graph": graph, "mode": "order", "order": None,
                        "pipe": None, "subject": subject, "root_query": q,
                        "focus": focus, "expand": ",".join(expand),
                        "problems_only": problems_only, "not_found": False}
        else:
            order = obj
            mode = mode or "order"
            subject = obj.order_number

    if not mode:
        mode = "live"

    if mode == "pipe" and pipe is not None:
        graph = traceability_service.build_pipe_graph(
            pipe, problems_only=problems_only)
    elif mode == "order" and order is not None:
        graph = traceability_service.build_graph(
            order=order, expand=expand, problems_only=problems_only)
    else:
        mode = "live"
        graph = traceability_service.build_live_graph(
            problems_only=problems_only)

    return {"graph": graph, "mode": mode, "order": order, "pipe": pipe,
            "subject": subject, "root_query": q, "focus": focus,
            "expand": ",".join(expand), "problems_only": problems_only,
            "not_found": not_found}


@reports_bp.route("/traceability-tree")
@login_required
@requires_permission("reports", "traceability_tree")
def traceability_tree():
    ctx = _traceability_context()
    orders = (ProductionOrder.query
              .order_by(ProductionOrder.id.desc()).limit(50).all())
    return render_template("reports/traceability_tree.html",
                           orders=orders, **ctx)


@reports_bp.route("/traceability-tree/canvas")
@login_required
@requires_permission("reports", "traceability_tree")
def traceability_tree_canvas():
    """The SVG fragment alone — expand/collapse swaps and the refresh poll."""
    ctx = _traceability_context()
    return render_template("reports/_traceability_canvas.html", **ctx)


@reports_bp.route("/traceability-tree/node/<kind>/<int:id>")
@login_required
@requires_permission("reports", "traceability_tree")
def traceability_tree_node(kind, id):
    """The inspector panel fragment for one node."""
    detail = traceability_service.node_detail(kind, id)
    if detail is None:
        return "<div class='p-3 text-muted'>—</div>", 404
    return render_template("reports/_traceability_node.html", d=detail)


@reports_bp.route("/mechanical-statistical")
@login_required
@requires_permission("reports", "mechanical_statistical")
...
@login_required
@requires_permission("reports", "stage_performance")
...
@login_required
@requires_permission("reports", "machine_performance")
...
@login_required
@requires_permission("reports", "mold_performance")
...
def capability():
    ...
    series = spc_service.build_series(characteristic, filters)
    values = [p["value"] for p in series["points"]]
    # Dimensional limits depend on the DN, so the filters have to reach the
    # resolver — a series spanning several DNs has no single tolerance band.
    limits = capability_service.spec_limits_for(characteristic, filters)
    ...
    data = {
        ...
        # Says which filter would make the limits resolvable, instead of
        # leaving a dimensional characteristic looking permanently broken.
        "limit_note": capability_service.dimensional_limit_note(
        ...
        "unavailable": [
            ...
            if capability_service.spec_limits_for(key, filters) is None
        ],
    }
...
@login_required
@requires_permission("reports", "lab_trends")
def lab_trends():
    filters = analytics_service.parse_filters(request.args)
    panels = []
...
@login_required
@requires_permission("reports", "stage_cycle_time")
...
@login_required
@requires_permission("reports", "stage_audit")
def stage_audit():
...
from app.services import ccm_panels_service, crosstab_service
...
@login_required
@requires_permission("reports", "ytd_comparison")
def ytd_comparison():
    today = date.today()
...
@login_required
@requires_permission("reports", "period_compare")
def period_compare():
    today = date.today()
...
@login_required
@requires_permission("reports", "stage_funnel")
def stage_funnel():
...
@login_required
@requires_permission("reports", "defect_heatmap")
def defect_heatmap():
...
@login_required
@requires_permission("reports", "saving_matrix")
def saving_matrix():
...
@login_required
@requires_permission("reports", "data_quality")
def data_quality():
...
@login_required
@requires_permission("reports", "diagnosis")
def diagnosis():
...
@login_required
@requires_permission("reports", "bi_dashboard")
def bi_dashboard():
    """CCM BI hub — sidebar-navigated panels over one filtered dataset."""
    filters = analytics_service.parse_filters(request.args)
    data = bi_service.bi_dashboard(filters)
    data["saving"] = bi_service.saving_matrix(filters)
    # Daily report section (auto-picks latest production date). It now takes
    # the page filters like every other panel, and an optional end date.
    rpt_date = _iso_date_arg("rpt_date")
    rpt_to = _iso_date_arg("rpt_to")
    data["daily_report"] = bi_service.daily_report(rpt_date, filters, rpt_to)

    # Panels + crosstab share one fact-table pass over the SAME filtered pipes.
    panels = ccm_panels_service.build(filters)
    facts = panels.pop("facts")
    data["panels"] = panels

    # xt_set marks a deliberate submission from the crosstab form. Without it
    # an absent xt_pivot/xt_rows means "first visit, use defaults"; with it, an
    # empty list means the user cleared every chip and wants no grouping.
    submitted = request.args.get("xt_set") == "1"
    data["crosstab"] = crosstab_service.crosstab(
        filters,
        pivot_dims=request.args.getlist("xt_pivot") if submitted
        else (request.args.getlist("xt_pivot") or ["stage"]),
        row_dims=request.args.getlist("xt_rows") if submitted
        ...
        sort=request.args.get("xt_sort", "total_desc"),
        row_limit=request.args.get("xt_limit", 20, type=int),
        display=request.args.get("xt_display", "full"),
        facts=facts,
        # xt_mset plays the same role as xt_set: it marks a real submission, so
        # unchecking every box falls back to the default set instead of being
        # read as "the form sent nothing".
        measures=(request.args.getlist("xt_m")
                  if request.args.get("xt_mset") == "1" else None),
    )
    data["crosstab_meta"] = {
        "dimensions": crosstab_service.dimension_choices(),
        "dimensions_ar": crosstab_service.dimension_choices("ar"),
        "presets": crosstab_service.preset_choices(),
        "presets_ar": crosstab_service.preset_choices("ar"),
        "sorts": crosstab_service.sort_choices(),
        "sorts_ar": crosstab_service.sort_choices("ar"),
        "measures": crosstab_service.measure_choices(),
        "measures_ar": crosstab_service.measure_choices("ar"),
    }
    data["filter_options"] = _bi_filter_options()
    ...
    data["payload"] = {
        ...
        "trends": data["trends"],
        "panels": {
            "stage_matrix": panels["stage_matrix"],
            "defects": panels["defects"],
            "reasons": panels["reasons"],
            "trends": panels["trends"],
        },
    }
    return _render_analytics_report("bi_dashboard.html", "CCM BI Dashboard", data)


@reports_bp.route("/export/daily-report-pdf")
@login_required
@requires_permission("reports", "bi_dashboard")
@requires_permission("reports", "export")
def export_daily_report_pdf():
    """Server-side PDF of the v52-style daily report."""
    # Accepts rpt_date/rpt_to like the page; `date` stays supported because
    # the two used to disagree and old links are in circulation.
    rpt_date = _iso_date_arg("rpt_date") or _iso_date_arg("date")
    rpt_to = _iso_date_arg("rpt_to")
    filters = analytics_service.parse_filters(request.args)
    report = bi_service.daily_report(rpt_date, filters, rpt_to)
    if not report:
        return redirect(url_for("reports.bi_dashboard"))
    from app.services.report_service import generate_daily_report_pdf

    pdf_buffer = generate_daily_report_pdf(report)
    return send_file(
        pdf_buffer,
        mimetype="application/pdf",
        as_attachment=True,
        download_name=f"daily_report_{report['date']}.pdf",
    )
...
@login_required
@requires_permission("reports", "weight_saving")
def weight_saving():
...
@login_required
@requires_permission("reports", "annealing_hourly")
...
@login_required
@requires_permission("reports", "management_dashboard")
...
@login_required
@requires_permission("reports", "shift_engineer")
def shift_engineer():
...
@login_required
@requires_permission("reports", "shift_engineer_comparison")
...
@login_required
@requires_permission("reports", "delivery_comparison")
...
@login_required
@requires_permission("reports", "delivery_overview")
...
@login_required
@requires_permission("reports", "stage_measurements")
...
@reports_bp.route("/pipe-measurements")
@login_required
@requires_permission("reports", "stage_measurements")
def pipe_measurements():
    """One pipe's measurement sheet — when it was made, every raw reading, and
    the statistics for each set.

    Takes ``pipe`` as an id or a pipe/no code so the report can be reached
    straight from a barcode scan or a link on the detail page.
    """
    raw = (request.args.get("pipe") or "").strip()
    pipe = None
    if raw:
        if raw.isdigit():
            pipe = Pipe.query.get(int(raw))
        if pipe is None:
            pipe = Pipe.query.filter(
                db.or_(Pipe.pipe_code == raw, Pipe.no_code == raw)
            ).first()

    sheet = (
        pipe_measurement_service.pipe_measurement_sheet(pipe.id)
        if pipe else None
    )
    data = {
        # The shared analytics base reads data.filters even though this report
        # keys off a single pipe rather than the standard filter bar.
        "filters": analytics_service.parse_filters(request.args),
        "query": raw,
        "sheet": sheet,
        "not_found": bool(raw) and sheet is None,
        # Recent pipes carrying any measurement, as a picker for the common
        # case where the operator does not have a code to hand.
        "recent": _recent_measured_pipes(),
        "export_rows": pipe_measurement_service.flatten_for_export(sheet),
    }
    return _render_analytics_report(
        "pipe_measurements.html", "Pipe Measurement Sheet", data
    )


def _recent_measured_pipes(limit=25):
    """Latest pipes that have at least one JSON measurement profile."""
    stages = (
        PipeStage.query.filter(
            db.or_(
                PipeStage.dimension_profile.isnot(None),
                PipeStage.thickness_profile.isnot(None),
                PipeStage.ovality_profile.isnot(None),
            )
        )
        .order_by(PipeStage.pipe_id.desc())
        .limit(limit * 4)
        .all()
    )
    seen = []
    for stage in stages:
        if stage.pipe_id in seen:
            continue
        seen.append(stage.pipe_id)
        if len(seen) >= limit:
            break
    if not seen:
        return []
    pipes = Pipe.query.filter(Pipe.id.in_(seen)).all()
    order = {pid: i for i, pid in enumerate(seen)}
    return sorted(pipes, key=lambda p: order.get(p.id, 0))


@reports_bp.route("/approvals")
@login_required
@requires_permission("reports", "approvals")
def approvals():
...
@reports_bp.route("/approver-comparison")
@login_required
@requires_permission("reports", "approver_comparison")
def approver_comparison():
    """Approvers side by side — the comparison the Approval Register cannot
    show, one row per approver with the accept/reject split and a per-DN
    breakdown."""
    filters = analytics_service.parse_filters(request.args)
    data = analytics_service.approver_comparison(filters)
    return _render_analytics_report(
        "approver_comparison.html", "Approver Comparison", data
    )
...
def export_analytics_excel(report_name):
    ...
    func_map = {
        ...
        "rework-report": analytics_service.rework_report,
        ...
        "approver-comparison": analytics_service.approver_comparison,
        # The BI dashboard's Excel button posts report_name "bi-dashboard".
        # It had no entry here, so the green button returned a 404 JSON body.
        "bi-dashboard": _crosstab_export,
    }
...
@login_required
@requires_permission("reports", "mechanical_properties")
...
def api_ai_summary(report_type):
    ...
    if report_type == "chemical":
        ...
        # Rebuild exactly what the panel rendered from the same query string,
        # then flatten it to text. The model sees the table, not the fact rows.
        export = _crosstab_export(analytics_service.parse_filters(request.args))
        rows = export["rows"]
        if not rows:
            return jsonify({"error": "No rows in the current selection"}), 404
        columns = list(rows[0].keys())
        data = {
            "columns": columns,
            "rows": [[r.get(c) for c in columns] for r in rows],
            "row_dims": request.args.getlist("xt_rows") or ["dn"],
            "pivot_dims": request.args.getlist("xt_pivot"),
            "measures": request.args.getlist("xt_m") or ["acc", "rej", "rej_pct"],
            "shown_rows": len(rows),
            "total_rows": export["summary"]["rows"],
            "coverage": {
                "shows_weight": any(
                    m in crosstab_service.WEIGHT_MEASURES
                    for m in request.args.getlist("xt_m")),
                "weighed": export["summary"]["pipes_with_both_weights"],
                "pipes": export["summary"]["pipes"],
            },
        }

    elif report_type == "daily":
        # Reuse the report the page is already showing rather than
        # recomputing: the model must analyse the same numbers the reader sees.
        rpt_date = _iso_date_arg("rpt_date")
        rpt_to = _iso_date_arg("rpt_to")
        filters = analytics_service.parse_filters(request.args)
        report = bi_service.daily_report(rpt_date, filters, rpt_to)
        if not report:
            return jsonify({"error": "No production data for that selection"}), 404
        today = report["today"]
        data = {
            "period": (f"{report['date_from']} — {report['date_to']}"
                       if report["is_range"] else report["date"]),
            "produced": today["produced"],
            "decided": today["n"],
            "rejects": today["rejects"],
            "rej_pct": today["rej_pct"],
            "saving": {"pct": today["sav_pct"], "n": today["sav_n"],
                       "of": today["sav_of"]},
            "stage_rows": report["stage_rows"],
            "top_reasons": report["top_reasons"],
            "defects": report["defects"]["rows"],
        }
