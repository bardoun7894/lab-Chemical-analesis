...
def _propagate_or_warn(mech_test):
    """Run decision propagation for an already-committed mechanical test.

    Callers commit the test BEFORE calling this, so a failure here can safely
    roll back the propagation's pending pipe changes without losing the test.

    This replaces a former ``except Exception: pass`` at every call site: a
    transient failure (e.g. a SQLite write-lock during propagation's own commit)
    used to be swallowed silently, flashing "success" while the pipes stayed
    stranded at WAITING with no error and no retry path. Now we log it and warn
    the user to retry so a stuck decision is visible and recoverable.
    """
    if not (mech_test.decision and mech_test.ladle_id):
        return
    from app.services.pipe_decision_service import propagate_mechanical_result
    try:
        propagate_mechanical_result(mech_test)
    except Exception:
        db.session.rollback()
        current_app.logger.exception(
            "propagate_mechanical_result failed for test %s (ladle %s)",
            mech_test.id, mech_test.ladle_id,
        )
        flash(
            "Test saved, but updating the ladle/lab decision failed — please "
            "re-open the test and save again to apply it.",
            "warning",
        )
...
def list():
    ...
    return render_template('mechanical/list.html',
                          picker_columns=export_service.picker_meta(table_registry.mechanical_columns()),
                          tests=tests,
                          grouped=grouped,
                          total=total,
...
@requires_permission('mechanical', 'list')
def export_excel():
    """Export/print filtered mechanical tests, respecting current filters.

    Honours ``cols=``, ``ids=`` and ``format=print`` from the Table Tools
    picker. Default set = the on-screen columns plus Shift/Tester/Status; the
    full measurement registry (dimensions, forces, microstructure …) lives in
    table_registry and is available via the picker.
    """
    from app.services import export_service, table_registry

    query, _ = _filtered_query(request.args)
    query = query.order_by(MechanicalTest.test_date.desc())
    query = export_service.filter_ids(query, MechanicalTest.id, request.args.get("ids"))
    rows = query.all()

    selected = export_service.resolve_columns(
        table_registry.mechanical_columns(), request.args.get("cols"))
    if request.args.get("format") == "print":
        return export_service.build_print("Mechanical Tests", selected, rows)
    return export_service.build_xlsx("Mechanical", selected, rows, "mechanical_tests.xlsx")
    ...
    if request.method == 'POST':
        try:
            ...
            if pipe_id:
                ...
                if pipe:
                    # Blocked (تالف) / terminally rejected pipes are not
                    # testable — the picker hides them; refuse a direct post
                    # too (DrAlaa 2026-08-10: X1 was selectable and testable).
                    if pipe.lab_decision == 'BLOCKED' or pipe.final_decision_value == 'REJECT':
                        flash(f"Pipe {pipe.pipe_code or pipe.no_code} is blocked/rejected (تالف) and cannot be mechanically tested.", 'error')
                        return redirect(url_for('mechanical.add'))
...
def api_pipes_search():
    ...
    query = Pipe.query
    # Lab-rejected / blocked (تالف) pipes ARE returned, but flagged so the
    # picker can show a red mark and disable them (DrAlaa 2026-08-13: show a
    # red mark so the lab doesn't select them). Selection is still refused
    # server-side on POST — the flag only drives the picker UI.
    if q:
        like = f"%{q}%"
    ...
    if annealing:
        try:
            ...
            annealed_ids = (db.session.query(PipeStage.pipe_id)
                            .filter(PipeStage.stage_name == annealing_name,
    ...
    if pipe_ids:
        ...
        for pid, sdate in (db.session.query(PipeStage.pipe_id, PipeStage.stage_date)
                           .filter(PipeStage.pipe_id.in_(pipe_ids),
                                   PipeStage.stage_name == annealing_name,
    ...
    results = [{
        ...
        'blocked': bool(p.lab_decision == 'BLOCKED' or p.final_decision_value == 'REJECT'),
