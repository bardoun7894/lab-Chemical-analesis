...
class StickerBarcodeDependencyTestCase(unittest.TestCase):
    ...
    def test_sticker_code128_helper_returns_image(self):
        # The renderer's own helper, not just the library: on 2026-09-17 the
        # QR/Data Matrix helper was pasted into the middle of _code128, which
        # then returned None for every value and both sticker barcodes vanished.
        from app.routes.stickers import _code128

        app = create_app("testing")
        with app.app_context():
            img = _code128("PIP1-1-112082026", 400, 120, "product")
        self.assertIsNotNone(img, "_code128 returned None for a valid value")
