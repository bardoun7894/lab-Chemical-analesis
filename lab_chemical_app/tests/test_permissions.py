...
class PermissionTestCase(unittest.TestCase):
    ...
    def _tamper(self):
        """Simulate someone playing with the matrix: revoke a default grant
        and hand a role a screen it should never have."""
        self._revoke("operator", "chemical", "add")
        perm = Permission.query.filter_by(module="admin", screen="users_delete").one()
        db.session.add(RolePermission(role="viewer", permission_id=perm.id))
        db.session.commit()

    def test_restore_default_grants_returns_matrix_to_role_defaults(self):
        self._tamper()
        self.assertFalse(has_permission("operator", "chemical", "add"))
        self.assertTrue(has_permission("viewer", "admin", "users_delete"))

        restore_default_grants()

        self.assertTrue(has_permission("operator", "chemical", "add"))
        self.assertFalse(has_permission("viewer", "admin", "users_delete"))
        # Full sweep: every (role, permission) matches ROLE_DEFAULTS exactly —
        # no leftover custom grants, no missing default grants.
        for perm in Permission.query.all():
            for role, config in ROLE_DEFAULTS.items():
                expected = config is True or (
                ...
                self.assertEqual(
                    expected, actual, f"{role} {perm.module}.{perm.screen}"
                )

    def test_restore_route_is_owner_only(self):
        self._login("admin")
        resp = self.client.post("/admin/permissions/restore-defaults")
        self.assertEqual(resp.status_code, 302)
        # Nothing was reset: the tampered state survives a denied call.
        self._tamper()
        self._login("admin")
        self.client.post("/admin/permissions/restore-defaults")
        self.assertTrue(has_permission("viewer", "admin", "users_delete"))

    def test_restore_route_resets_after_tamper(self):
        self._tamper()
        self._login("super_admin")
        resp = self.client.post("/admin/permissions/restore-defaults")
    ...
    def test_matrix_page_renders_the_restore_button(self):
        self._login("super_admin")
        html = self.client.get("/admin/permissions").get_data(as_text=True)
        self.assertIn('id="restore-defaults"', html)
        self.assertIn("restore-defaults", html)  # route url referenced by the JS
