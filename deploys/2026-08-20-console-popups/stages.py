...
def _parse_thickness_profile(items, stage_name):
    """Scan a key/value mapping for the per-meter cement/coating thickness
    grid. Only the Coating row renders these inputs. Returns
    (profile_dict_or_None, has_any_value). Profile shape:
    {"cement": [m1..m6], "coating": [m1..m6]} with None for empty cells.
    """
    ...
    for layer in ("cement", "coating"):
        vals = []
        for m in range(1, 7):
            raw = values.get(f"stage_{stage_name}_thick_{layer}_{m}")
            if raw:
                has_any = True
                ...
                vals.append(None)
        profile[layer] = vals
    return (profile if has_any else None), has_any
...
def _parse_dimension_profile(items, stage_name):
    """Scan a key/value mapping for the CCM dimension grids (thickness 1-21,
    diameter D1-D15).

    Input names: stage_<CCM>_dim_thick_S<k>_<n>, stage_<CCM>_dim_dia_S<k>_<n>.
    Sample rows are dynamic (S1..Sn) so keys are scanned rather than
    enumerated. The legacy "Standard" thickness row was removed 2026-07-26 —
    any dim_thick_standard_* input is ignored and saved profiles no longer
    carry a "standard" key (old data in existing profiles is simply not
    rendered anymore). Returns (profile_or_None, has_any).
    """
    ...
    for key, val in items:
        ...
        if m and 1 <= int(m.group(2)) <= 21:
            thick["samples"].setdefault(m.group(1), [None] * 21)[
                int(m.group(2)) - 1
            ] = float(val)
            has_any = True
            continue
    ...
    if not has_any:
        return None, False
    return {"thickness": thick, "diameter": dia}, True
...
OVALITY_POINTS = ["ID"] + [f"D{n}" for n in range(1, 16)]


def _parse_ovality_profile(items, stage_name):
    """Scan a key/value mapping for the Annealing ovality grid (ID, D1-D15).

    Input names: stage_<Annealing>_ov_x_<point>, stage_<Annealing>_ov_y_<point>,
    where <point> is one of OVALITY_POINTS. Ovality % is computed and stored
    per point as (x - y) / (x + y) * 100 so it does not need recomputing on
    every render (DrAlaa 2026-08-14 — popup on the Annealing stage).
    ...
    for key, val in items:
        if not val or not key.startswith(prefix):
            continue
        rest = key[len(prefix):]
        m = re.fullmatch(r"(x|y)_(ID|D\d+)", rest)
        if not m or m.group(2) not in OVALITY_POINTS:
            continue
        ...
        try:
            ...
            for stage_name in stage_names:
                ...
                if (
                    ...
                    # Per-meter thickness profile for Coating (blank keeps prior)
                    if has_thickness:
                        stage.thickness_profile = thickness_profile
